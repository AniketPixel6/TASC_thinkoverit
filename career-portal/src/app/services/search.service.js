        ...
        return this._.requestParams || (this._.requestParams = {
                ...
                count: () => {
                    ...
                    if (!this.searchParams.reloadAllData) {
                        count = this.searchParams.loadCount || SearchService._loadCount;
                    }
                    else {
                        if (this.searchParams.filteredCategory || this.searchParams.getHired) {
                            count = this.searchParams.loadCount || SearchService._loadCount;
                        }
                        else {
                ...
                employmentType: (isSearch, fields) => {
                    if ('employmentType' !== fields) {
                        if (this.searchParams.employmentType.length > 0) {
                            ...
                            for (let j = 0; j < this.searchParams.employmentType.length; j++) {
                                if (!first) {
                                    fragment += ' OR ';
                                } else {
                                    first = false;
                                ...
                                let employmentType = this.searchParams.employmentType[j];
                ...
                publishedDate: () => {
                    if (this.searchParams.dateSearch) {
                        let ms = Date.parse(this.searchParams.dateSearch);
                        console.log(ms);
                        let ms1 = ms;
                        let ms2 = 43200000 + ms;
